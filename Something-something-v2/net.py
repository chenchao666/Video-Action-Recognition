...
class InceptionI3d(snt.AbstractModule):
  ...
  def _build(self, inputs, is_training, dropout_keep_prob=1.0):
    ...
    # net = Unit3D(output_channels=64, kernel_shape=[7, 1, 1], stride=[1, 2, 2], name='Conv3d_1a_7x1x1')(net,is_training=is_training)
    ...
    net = tf.nn.max_pool3d(net, ksize=[1, 3, 3, 3, 1], strides=[1, 1, 2, 2, 1], padding=snt.SAME, name=end_point)  # strides=[1, 2, 2, 2, 1]
    end_points[end_point] = net
    if self._final_endpoint == end_point: return net, end_points
